...
namespace DataLayer
{
    ...
    public class SPECCPUHelper
    {
        ...
        public void SaveNewExperiment(string dbFile, string url, string CpuName, string CpuMHz, string FPU, string Memory, string OS, string Compiler, string ProgramName, decimal BaseRefTime, decimal BaseRunTime, decimal PeakRefTime, decimal PeakRunTime, string kind)
        {
            using (var db = new SPECCPUDB_Mine(new LinqToDB.DataProvider.SQLite.SQLiteDataProvider(), String.Format(@"Data Source={0}", dbFile)))
            {
                ...
                if (expID < 0)
                {
                    ...
                    exp.PeakRefTime = PeakRefTime;
                    exp.PeakRunTime = PeakRunTime;
                    exp.kind = kind;
                    db.Insert(exp);
                }
            }
        ...
        public Experiment[] getExperimentsOfEnvironmentFilterByKind(SPECCPUDB_Mine db, long ID, string kind)
        {
            return db.Experiments.Where(e => e.EnvID == ID && e.kind.Equals(kind)).OrderBy(e => e.ProgID).ToArray();
        }
